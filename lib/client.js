...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const btn = { fontSize: "12px", padding: "4px 10px", borderRadius: "8px", border: "1px solid var(--color-border, rgba(148,163,184,0.3))", background: "transparent", color: "var(--color-text-2, #94a3b8)", cursor: "pointer" };
    // 面板用 portal 挂到 document.body，fixed 定位到视口，避开侧栏列 overflow:hidden 裁剪
    const panelStyle = { position: "fixed", bottom: "88px", width: "min(460px, calc(100vw - 32px))", maxHeight: "calc(100vh - 170px)", overflowY: "auto", padding: "14px 16px", borderRadius: "12px", background: "var(--color-surface-2, #171b2e)", border: "1px solid var(--color-border, rgba(148,163,184,0.18))", color: "var(--color-text-1, #e2e8f0)", fontSize: "13px", zIndex: 999, boxShadow: "0 12px 32px rgba(0,0,0,0.35)" };
    ...
    function WorkbenchPanel(props) {
      ...
      return react.createElement("div", { style: { ...panelStyle, ...panelSide() } },
        ...
        react.createElement("div", { style: { display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "8px" } },
          tabs.map(([id, label]) => react.createElement("button", { key: id, style: { ...btn, fontWeight: tab === id ? 700 : 400, borderBottom: tab === id ? "2px solid var(--color-accent, #6366f1)" : "2px solid transparent" }, onClick: () => setTab(id) }, label))),
        tab === "usage" && UsageTab({ usage }),
        tab === "skills" && SkillsTab({ status: null, fetchStatus: true }),
        tab !== "usage" && tab !== "skills" && react.createElement("div", { style: { color: "var(--color-text-3, #64748b)", padding: "12px 0" } },
          "「" + (tabs.find((t) => t[0] === tab) || ["", ""])[1] + "」区块将在 M2/M3 里程碑实装。")
      );
    }

    // ---------- 使用说明（两层分类：阵营 → 小类 → 口令） ----------
    const CAMP_ACCENT = { st: "var(--color-accent, #6366f1)", vibe: "#9d5f4d" };
    ...
    function UsageCamp({ camp, expanded, onToggle }) {
      const header = react.createElement("button", { onClick: onToggle, style: { width: "100%", textAlign: "left", background: "transparent", border: "none", padding: "8px 10px", borderRadius: "8px", cursor: "pointer" } },
        react.createElement("div", { style: { display: "flex", alignItems: "center", gap: "8px" } },
          ...
          react.createElement("strong", { style: { fontSize: "13px", color: "var(--color-text-1, #e2e8f0)" } }, camp.campLabel),
          react.createElement("span", { style: { fontSize: "11px", color: "var(--color-text-3, #64748b)", marginLeft: "auto", marginRight: "4px" } }, camp.memo),
          react.createElement("span", { style: { color: "var(--color-text-3, #64748b)", fontSize: "12px" } }, expanded ? "▾" : "▸")),
        !expanded && react.createElement("div", { style: { paddingLeft: "24px", marginTop: "2px" } },
          react.createElement("span", { style: { fontSize: "11px", color: "var(--color-text-3, #64748b)" } }, camp.groups.map((g) => g.label).join(" · ")))
      );
      const body = expanded && react.createElement("div", { style: { paddingTop: "4px" } },
        camp.groups.map((group) => react.createElement("div", { key: group.label, style: { marginBottom: "8px" } },
          react.createElement("div", { style: { display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px", padding: "0 10px" } },
            react.createElement("span", { style: { fontSize: "11px", color: CAMP_ACCENT[camp.camp], fontWeight: 500, flexShrink: 0 } }, group.label),
            react.createElement("span", { style: { flex: 1, height: "1px", background: "var(--color-border, rgba(148,163,184,0.18))" } })),
          group.items.map((u) => react.createElement("div", { key: u.scene, style: { border: "1px solid var(--color-border, rgba(148,163,184,0.18))", borderRadius: "8px", padding: "8px 10px", marginBottom: "5px" } },
            react.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "4px" } },
              react.createElement("strong", { style: { fontSize: "12px", color: "var(--color-text-1, #e2e8f0)" } }, u.scene),
              react.createElement("button", { style: { ...btn, padding: "2px 8px", fontSize: "11px", color: "var(--color-accent, #6366f1)" }, title: "复制发送内容", onClick: () => copyText(u.send) }, "复制")),
            react.createElement("div", { style: { fontFamily: "monospace", fontSize: "11px", whiteSpace: "pre-wrap", color: "var(--color-text-2, #94a3b8)", marginBottom: "3px" } }, u.send.length > 120 ? u.send.slice(0, 120) + "…" : u.send),
            u.detail && react.createElement("div", { style: { color: "var(--color-text-3, #64748b)", fontSize: "11px" } }, u.detail))))
        )
      );
    ...
    function UsageTab({ usage }) {
      ...
      if (usage.length === 0) return react.createElement("div", { style: { color: "var(--color-text-3, #64748b)" } }, "未取得使用说明数据。");
      return react.createElement("div", null,
        react.createElement("div", { style: { color: "var(--color-text-3, #64748b)", fontSize: "11px", marginBottom: "8px" } }, "源自官方 newbie-guide 复制口令 · 按阵营分层 · 点条目复制发送内容"),
    ...
    function SkillsTab({ status, fetchStatus }) {
      ...
      if (st.total === 0) return react.createElement("div", { style: { color: "var(--color-text-3, #64748b)" } }, "未找到 TavernWeave 技能目录。请确认仓库位置或安装技能。");
      return react.createElement("div", null,
        react.createElement("div", { style: { marginBottom: "8px" } },
          "技能安装状态：", react.createElement("strong", null, st.installed + "/" + st.total),
          st.missing.length > 0 && react.createElement("span", { style: { color: "#f59e0b" } }, "（缺失: " + st.missing.join(", ") + "）"),
          react.createElement("div", { style: { color: "var(--color-text-3, #64748b)", fontSize: "11px", marginTop: "2px" } }, "官方状态：离线候选（未做真实 SillyTavern 验收）")),
        react.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr", gap: "4px" } },
          st.skills.map((s) => react.createElement("div", { key: s.name, style: { display: "flex", alignItems: "center", gap: "6px" } },
            react.createElement("span", { style: { color: s.ok ? "#10b981" : "#ef4444" } }, s.ok ? "✓" : "✗"),
            react.createElement("span", { style: { fontFamily: "monospace", fontSize: "12px" } }, s.name))))
      );
    }
    ...
    function SettingsForm() {
      ...
      const label = { display: "block", fontSize: "12px", color: "var(--color-text-2, #94a3b8)", marginBottom: "4px", marginTop: "10px" };
      const input = { width: "100%", boxSizing: "border-box", padding: "6px 8px", borderRadius: "8px", border: "1px solid var(--color-border, rgba(148,163,184,0.3))", background: "var(--color-surface-1, #101423)", color: "var(--color-text-1, #e2e8f0)", fontSize: "12px" };
      const tm = s.tavernModel || {};
      return react.createElement("div", { style: { maxWidth: "560px" } },
        react.createElement("div", { style: { fontSize: "11px", color: "var(--color-text-3, #64748b)", marginBottom: "2px" } }, "入口显示：白名单非空时才显示「🍺 酒馆」按钮"),
        ...
        react.createElement("div", { style: { fontSize: "12px", fontWeight: 600, marginTop: "14px", marginBottom: "2px" } }, "侧栏模型（模拟酒馆测试用，M3 实装）"),
        ...
        react.createElement("div", { style: { display: "flex", alignItems: "center", gap: "10px", marginTop: "16px" } },
          react.createElement("button", { style: { ...btn, color: "var(--color-text-1, #e2e8f0)", border: "1px solid var(--color-accent, #6366f1)", padding: "6px 16px" }, onClick: save }, "保存"),
          msg && react.createElement("span", { style: { fontSize: "12px", color: msg === "已保存" ? "#10b981" : "#ef4444" } }, msg))
      );
    }
