  ...
  factory: (require) => {
    ...
    function WorkshopTab() {
      ...
      const copyMaterialSummary = () => {
        if (!material) return;
        const doneCount = material.chapterList.filter((ch) => ch.status === "done").length;
      ...
      const toggleChapter = (index) => {
        if (!material) return;
        const chapterList = material.chapterList.map((ch, i) => i === index ? { ...ch, status: ch.status === "done" ? "pending" : "done" } : ch);
      ...
      const copyChapterTask = (index) => {
        if (!material) return;
        const ch = material.chapterList[index];
        if (!ch) return;
      ...
      const copyNextPending = () => {
        if (!material) return;
        const idx = material.chapterList.findIndex((ch) => ch.status !== "done");
      ...
      return react.createElement("div", null,
        ...
        react.createElement("div", { style: { marginBottom: "11px", padding: "10px", borderRadius: DSGN.radius.md, border: "1px solid #65d2ff44", background: "linear-gradient(135deg, rgba(101,210,255,.1), transparent 75%)" } },
          ...
          material && react.createElement("div", { style: { marginTop: "8px", padding: "8px", borderRadius: DSGN.radius.sm, background: "var(--dsw-alias-bg-module-platform)", border: "1px solid rgba(101,210,255,.18)" } },
            ...
            material.chapterList && react.createElement("div", { style: { marginTop: "8px", paddingTop: "8px", borderTop: "1px solid var(--dsw-alias-border-l1)" } },
              react.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: "8px", marginBottom: "6px", flexWrap: "wrap" } },
                react.createElement("span", { style: { fontSize: "11px", color: "var(--dsw-alias-label-secondary)", fontWeight: 600 } }, "章节大纲（" + material.chapterList.filter((ch) => ch.status === "done").length + "/" + material.chapterList.length + " 已确认）"),
              ...
              react.createElement("div", { style: { display: "grid", gap: "4px", maxHeight: "180px", overflowY: "auto" } },
                material.chapterList.map((ch, i) => react.createElement("div", { key: i, style: { display: "flex", alignItems: "center", gap: "6px", minHeight: "24px" } },
                  react.createElement("span", { style: { width: "6px", height: "6px", borderRadius: "50%", flexShrink: 0, background: ch.status === "done" ? "#74e0b2" : "#f0c879" } }),
                  react.createElement("span", { style: { flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: "11px" } }, (i + 1) + ". " + ch.title + "（" + ch.chars + " 字）"),
                  ...
                  react.createElement("button", { type: "button", style: { ...btn, minHeight: "22px", padding: "1px 6px", fontSize: "10px", color: ch.status === "done" ? "var(--dsw-alias-state-success-primary)" : "var(--dsw-alias-label-tertiary)" }, onClick: () => toggleChapter(i) }, ch.status === "done" ? "✓ 已确认" : "确认"))))),
