...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const TAB_HINT = {
      usage: "不会用时先从这里找一句合适的口令，再复制到当前会话。",
      skills: "这里显示插件目录是否完整，以及当前会话实际加载过的 Skill。",
      library: "这里是原作者指南的只读索引，用来查概念、流程和实现边界。",
      workshop: "这里管理角色卡草稿：导入、结构盘点、组件修改、版本记录和导出。",
      updates: "这里只更新 TavernWeave DSH 插件本身，不会覆盖您的卡片项目。",
    };
    ...
    const panelStyle = { position: "fixed", top: "24px", bottom: "auto", width: "min(520px, calc(100vw - 24px))", maxHeight: "min(700px, calc(100dvh - 48px))", overflow: "hidden", borderRadius: "18px", background: "var(--dsw-specific-menu, var(--dsw-specific-input-major, var(--dsw-alias-bg-base)))", border: "1px solid var(--dsw-alias-border-inverted)", color: "var(--dsw-alias-label-primary)", fontSize: "13px", zIndex: 999, boxShadow: "var(--dsw-shadow-lv3)", backdropFilter: "blur(14px) saturate(1.08)", WebkitBackdropFilter: "blur(14px) saturate(1.08)" };
    ...
    function WorkbenchPanel(props) {
      ...
      const moveDrag = (event) => {
        ...
        setDragPosition({
          left: Math.min(maxLeft, Math.max(8, drag.left + event.clientX - drag.startX)),
          top: Math.min(maxTop, Math.max(8, drag.top + event.clientY - drag.startY)),
        });
        event.preventDefault();
      };
      const endDrag = () => { dragRef.current = null; setDragging(false); };
      const positionStyle = dragPosition ? { left: dragPosition.left + "px", top: dragPosition.top + "px", right: "auto", bottom: "auto" } : {};
      return react.createElement("div", { role: "dialog", "aria-label": "TavernWeave 酒馆工作台", onPointerDown: beginDrag, onPointerMove: moveDrag, onPointerUp: endDrag, onPointerCancel: endDrag, style: { ...panelStyle, ...panelSide(), ...positionStyle, cursor: dragging ? "grabbing" : "grab", touchAction: dragging ? "none" : "auto" } },
      ...
      return react.createElement("div", { style: { display: "inline-flex", alignItems: "center", gap: "5px", marginLeft: "4px" }, "aria-label": "Soul 人格选择" },
        ...
        SOUL_PERSONAS.map((persona) => {
          ...
          return react.createElement("span", { key: persona.id, style: { position: "relative", display: "inline-flex" },
            ...
            hovered === persona.id && react.createElement("div", { role: "tooltip", style: { position: "absolute", zIndex: 1001, left: "50%", bottom: "calc(100% + 7px)", transform: "translateX(-50%)", width: "min(270px, calc(100vw - 30px))", maxHeight: "min(320px, 45dvh)", overflowY: "auto", padding: "7px", borderRadius: "10px", border: "1px solid " + persona.tone + "55", background: "var(--dsw-specific-menu, var(--dsw-alias-bg-base))", color: "var(--dsw-alias-label-primary)", boxShadow: "var(--dsw-shadow-lv2)", fontSize: "11px", lineHeight: 1.35, pointerEvents: "auto" } },
    ...
    function ConversationDock({ useInput, inputActions }) {
      ...
      return react.createElement("div", { style: { position: "relative", display: "flex", alignItems: "center", alignSelf: "flex-end", justifyContent: "flex-end", flexWrap: "wrap", gap: "6px", padding: "5px 7px", marginTop: "6px", marginLeft: "auto", width: "fit-content", maxWidth: "100%", borderRadius: "10px", border: "1px solid var(--dsw-alias-border-l1)", background: "var(--dsw-alias-bg-module-platform)", color: "var(--dsw-alias-label-secondary)", fontSize: "12px", boxSizing: "border-box" } },
        ...
        notice && react.createElement("span", { role: "status", style: { position: "absolute", right: 0, top: "calc(100% + 4px)", maxWidth: "min(360px, 90vw)", padding: "4px 7px", borderRadius: "6px", background: "var(--dsw-specific-menu, var(--dsw-alias-bg-base))", color: "var(--dsw-alias-state-success-primary)", fontSize: "11px", zIndex: 2 } }, notice));
    }
    ...
    function SkillsTab({ status, fetchStatus }) {
      ...
      return react.createElement("div", null,
        react.createElement("div", { style: { color: "var(--dsw-alias-label-tertiary)", fontSize: "12px", lineHeight: 1.5, marginBottom: "8px" } }, "可用数量是已安装的能力；已加载数量只统计本会话真正调用过的 Skill。没有已加载项并不代表插件失效，模型会按任务需要加载。"),
    ...
    function LibraryTab() {
      ...
      const openDoc = (item) => {
        ...
        fetch("/tavernweave/library/doc?domain=" + encodeURIComponent(item.domain) + "&file=" + encodeURIComponent(item.file))
          ...
          .then((data) => {
            ...
            setMessage("");
          }).catch((error) => setMessage("无法读取该条目：" + (error.message || "未知错误")));
      };
      ...
      return react.createElement("div", null,
        react.createElement("div", { style: { color: "var(--dsw-alias-label-tertiary)", fontSize: "12px", lineHeight: 1.5, marginBottom: "8px" } }, "这里收录原作者的制卡规范、MVU、世界书、正则、开场和界面指南。先搜索并阅读，再把结论交给 DSH 制卡工作台；资料只读，不会自动修改您的卡片。"),
        ...
        react.createElement("div", { style: { display: "flex", alignItems: "center", gap: "5px", flexWrap: "wrap", marginBottom: "9px" } },
          react.createElement("span", { style: { color: "var(--dsw-alias-label-tertiary)", fontSize: "11px" } }, "快速查："),
        ...
        selected && react.createElement("section", { style: { marginBottom: "10px", padding: "10px", borderRadius: "10px", border: "1px solid var(--dsw-alias-border-l2)", background: "var(--dsw-alias-bg-base)" } },
          ...
          react.createElement("pre", { style: { margin: 0, maxHeight: "min(360px, 45dvh)", overflow: "auto", padding: "9px", borderRadius: "8px", background: "var(--dsw-alias-bg-module-platform)", color: "var(--dsw-alias-label-secondary)", whiteSpace: "pre-wrap", fontFamily: "inherit", fontSize: "12px", lineHeight: 1.55 } }, selected.content)),
    ...
    function WorkshopTab() {
      ...
      return react.createElement("div", null,
        react.createElement("div", { style: { marginBottom: "11px", padding: "8px 10px", borderRadius: "9px", background: "var(--dsw-alias-bg-module-platform)", color: "var(--dsw-alias-label-tertiary)", fontSize: "11px", lineHeight: 1.5 } }, "第一次使用：先点“从零制作新卡”或“改造已有卡”复制首轮指令，再到 DSH 会话中发送；导入 JSON 后可在这里盘点结构并做组件级修改。"),
