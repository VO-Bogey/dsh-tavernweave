...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const SOUL_PERSONAS = [
      { id: "atong", name: "阿瞳", glyph: "瞳", tone: "#74e0b2", command: "阿瞳助我！", hint: "温柔指导" },
      { id: "mttt", name: "MTTT.sir", glyph: "MT", tone: "#bca4ff", command: "MTTT.sir，拷打我！", hint: "严格学习" },
      { id: "soulkiller", name: "灵魂杀手", glyph: "✦", tone: "#f39ab7", command: "灵魂杀手！", hint: "前端审查" },
    ];
      ...
      return react.createElement("div", { style: { display: "inline-flex", alignItems: "center", gap: "5px", marginLeft: "4px" }, "aria-label": "Soul 人格选择" },
        ...
        SOUL_PERSONAS.map((persona) => {
          ...
          return react.createElement("span", { key: persona.id, style: { position: "relative", display: "inline-flex" },
            ...
            react.createElement("button", { type: "button", "aria-label": persona.name + "：" + persona.command, "aria-pressed": active, title: persona.name + " · " + persona.hint, onClick: () => choose(persona), style: { width: "30px", height: "30px", padding: 0, borderRadius: "50%", border: active ? "2px solid " + persona.tone : "1px solid var(--dsw-alias-border-l2)", background: active ? persona.tone + "2b" : "var(--dsw-alias-bg-module-platform)", color: active ? persona.tone : "var(--dsw-alias-label-secondary)", fontSize: persona.id === "mttt" ? "9px" : "13px", fontWeight: 700, cursor: "pointer", boxShadow: active ? "0 0 12px " + persona.tone + "55" : "none" } }, persona.glyph),
            hovered === persona.id && react.createElement("div", { role: "tooltip", style: { position: "absolute", zIndex: 1001, left: "50%", bottom: "calc(100% + 7px)", transform: "translateX(-50%)", width: "max-content", maxWidth: "220px", padding: "6px 8px", borderRadius: "8px", border: "1px solid " + persona.tone + "55", background: "var(--dsw-specific-menu, var(--dsw-alias-bg-base))", color: "var(--dsw-alias-label-primary)", boxShadow: "var(--dsw-shadow-lv2)", fontSize: "11px", lineHeight: 1.35, whiteSpace: "pre-line", pointerEvents: "none" } }, persona.name + " · " + persona.hint + "\nSoul口令：" + persona.command));
        })
      );
    }
    ...
    function ConversationDock({ input, inputActions }) {
      const visible = useVisible();
      ...
      return react.createElement("div", { style: { position: "relative", display: "flex", alignItems: "center", flexWrap: "wrap", gap: "7px", padding: "7px 9px", marginBottom: "6px", borderRadius: "10px", border: "1px solid var(--dsw-alias-border-l1)", background: "var(--dsw-alias-bg-module-platform)", color: "var(--dsw-alias-label-secondary)", fontSize: "12px" } },
