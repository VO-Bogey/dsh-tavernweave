...
// 插槽：conversation.input.dock（输入区入口）+ sidebar.footer.action（侧栏入口）
//      + settings.section（设置页）。两个入口共享同一面板开关与轮询状态（不会双开）。
      ...
      return react.createElement("div", { style: { ...panelStyle, ...panelSide() } },
        react.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" } },
          ...
          react.createElement("span", null,
            react.createElement("button", { style: { ...btn, marginRight: "6px" }, title: "设置入口", onClick: () => setHint("设置入口：DSH 左下角「设置」→ TavernWeave 分区") }, "⚙"),
            react.createElement("button", { style: btn, onClick: props.onClose }, "×"))),
        hint && react.createElement("div", { style: { fontSize: "12px", color: "var(--dsw-alias-state-warn-primary)", marginBottom: "6px" } }, hint),
        react.createElement("div", { style: { display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "8px" } },
          tabs.map(([id, label]) => react.createElement("button", { key: id, style: { ...btn, fontWeight: tab === id ? 700 : 400, borderBottom: tab === id ? "2px solid var(--dsw-alias-state-business-primary)" : "2px solid transparent" }, onClick: () => { setTab(id); setHint(""); } }, label))),
