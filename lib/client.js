    ...
    function currentWorkspacePath() {
      try {
        ...
        const currentId = sessionState?.current
          ...
          || "";
        const row = currentId ? (sessionState.byId?.[currentId] || (sessionState.items || []).find((item) => item.sessionId === currentId)) : null;
        ...
        const selected = selectedId ? items.find((item) => item.workspaceId === selectedId || item.id === selectedId) : null;
        const owned = currentId ? items.find((item) => (item.sessionIds || []).includes(currentId)) : null;
        return normalizeWorkspacePath(selected?.path || owned?.path || row?.workspacePath || row?.cwd || "");
      } catch (e) { return ""; }
    }
