...
- **更新中心**：会检查插件仓库与原作者资料库的远端 Git 状态。侧栏「✦ 酒馆」按钮会在发现可安全快进的更新时显示“有新更新”气泡；点击后可查看最近提交并手动执行一键安全更新。检测到本地未提交改动、仓库分叉或远端不可达时只提示，不覆盖本地内容。

资料库当前已经来自原作者 TavernWeave 仓库的 Git 工作副本（与插件同级的 `../TavernWeave`），不是临时复制的几篇 Markdown。更新中心会单独检查它的 `origin/main`；资料库更新不会改动插件代码或角色卡项目。
