...
- **更新中心**：只面向用户检查和更新本插件的远端仓库。侧栏「✦ 酒馆」按钮会在发现可安全快进的插件更新时显示“有新更新”气泡；点击后可查看最近提交并手动执行一键安全更新。检测到本地未提交改动、仓库分叉或远端不可达时只提示，不覆盖本地内容。

资料库当前来自原作者 TavernWeave 仓库的 Git 工作副本（与插件同级的 `../TavernWeave`），不是临时复制的几篇 Markdown。原作者更新由维护者拉取、审查并合并到本项目适配版本；用户只需要更新本插件，不需要直接操作原作者仓库。
